...
public class SkriptVault extends SimpleExpression<Double> {
    ...
    protected Double[] get(Event e) {
        ...
        return new Double[]{SVault.getStatsManager().getStats(p)};
    }
    ...
    public void change(Event e, Object[] delta, Changer.ChangeMode mode) {
        ...
        if (mode == Changer.ChangeMode.ADD) {
            SVault.getStatsManager().setStats(p, SVault.getStatsManager().getStats(p) + num);
        } else if (mode == Changer.ChangeMode.REMOVE) {
            SVault.getStatsManager().setStats(p, SVault.getStatsManager().getStats(p) - num);
        } else if (mode == Changer.ChangeMode.SET) {
            SVault.getStatsManager().setStats(p, num);
        }
